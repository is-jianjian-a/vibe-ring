...
Vibe Ring releases are code-signed and notarized via GitHub Actions. This document explains how to set up the required secrets.
...
2. Generate a new password, label it `vibe-ring-notary`
...
```bash
export VIBE_RING_SIGN_IDENTITY="Developer ID Application: Your Name (TEAMID)"
export VIBE_RING_NOTARY_PROFILE="vibe-ring-notary"
export VIBE_RING_VERSION="0.2.0"

# First, store notarization credentials (one-time):
xcrun notarytool store-credentials "vibe-ring-notary" \
