...
@MainActor
final class VibeRingAppDelegate: NSObject, NSApplicationDelegate {
    let model = AppModel()
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ProcessInfo.processInfo.disableAutomaticTermination(
            "Vibe Ring should remain active while monitoring local agent sessions."
        )
        ...
        DispatchQueue.main.async { [self] in
            ...
            VibeRingAppDelegate.hideAllAppWindows()
...
@main
struct VibeRingApp: App {
    @NSApplicationDelegateAdaptor(VibeRingAppDelegate.self)
