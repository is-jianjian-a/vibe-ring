...
struct SessionStateTests {
    ...
    func actionableStateResolvedClearsWaitingForApproval() {
        ...
        #expect(state.session(id: "claude-approval")?.summary == "Approval was handled outside Vibe Ring.")
    }
    ...
    func codexHookInstallerMergesManagedGroupsWithoutDroppingUnrelatedHooks() throws {
        ...
        let managedStopHook = stopGroups?
            .compactMap { $0["hooks"] as? [[String: Any]] }
            .flatMap { $0 }
            .first(where: { $0["command"] as? String == "'/tmp/VibeRingHooks'" })

        #expect(stopCommands.contains("/usr/bin/true"))
        #expect(stopCommands.contains("'/tmp/VibeRingHooks'"))
    ...
    @Test
    func codexHookInstallerInstallReplacesLegacyVibeRingCommands() throws {
        let existing = """
        {
          "hooks": {
            "PreToolUse": [
            ...
            "Stop": [
              {
                "hooks": [
                  ...
                  {
                    "type": "command",
                    "command": "'/tmp/old-debug/VibeRingHooks'"
                  },
                  {
                    "type": "command",
                    "command": "/usr/bin/true"
        ...
        let mutation = try CodexHookInstaller.installHooksJSON(
            existingData: existing,
            hookCommand: "'/tmp/new-release/VibeRingHooks'"
        )
        ...
        #expect(permissionCommands == ["'/tmp/new-release/VibeRingHooks'"])
        #expect(hooks?["PostToolUse"] == nil)
        #expect(stopCommands.contains("/usr/bin/true"))
        #expect(stopCommands.contains("'/tmp/new-release/VibeRingHooks'"))
        #expect(!stopCommands.contains("'/Users/test/.open-island/bin/open-island-bridge' --source codex"))
        #expect(!stopCommands.contains("'/tmp/old-debug/VibeRingHooks'"))
    }
    ...
    func codexHookInstallerUninstallRemovesOnlyManagedHooks() throws {
            ...
            "Stop": [
              {
                "hooks": [
                  {
                    "type": "command",
                    "command": "'/tmp/VibeRingHooks'",
                    "statusMessage": "Managed by Vibe Ring"
                  },
                  {
                    "type": "command",
                    "command": "/usr/bin/true",
        ...
        let mutation = try CodexHookInstaller.uninstallHooksJSON(
            existingData: existing,
            managedCommand: "'/tmp/VibeRingHooks'"
        )
    ...
    func codexHookInstallationManagerRoundTripsInstallAndUninstall() throws {
        ...
        let managedHooksBinaryURL = rootURL
            ...
            .appendingPathComponent("VibeRingHooks")
