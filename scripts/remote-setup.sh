...
HOOK_SCRIPT="$SCRIPT_DIR/vibe-ring-hooks.py"
REMOTE_BIN_DIR=".local/bin"
REMOTE_HOOK_PATH="\$HOME/$REMOTE_BIN_DIR/vibe-ring-hooks.py"
...
echo "==> Deploying vibe-ring-hooks.py to $REMOTE ..."
ssh "$REMOTE" "mkdir -p ~/$REMOTE_BIN_DIR"
scp "$HOOK_SCRIPT" "$REMOTE:~/$REMOTE_BIN_DIR/vibe-ring-hooks.py"
ssh "$REMOTE" "chmod +x ~/$REMOTE_BIN_DIR/vibe-ring-hooks.py"
...
# forwarded socket created by the local Vibe Ring app.  The heredoc is
# unquoted so that $SOCKET_NAME is expanded.
HOOK_CMD="VIBE_RING_SOCKET_PATH=/tmp/$SOCKET_NAME python3 ~/.local/bin/vibe-ring-hooks.py --source claude"
