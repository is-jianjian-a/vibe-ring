...
/// Installs/uninstalls Vibe Ring's managed `[[hooks]]` entries in
/// `~/.kimi/config.toml`.
///
...
public enum KimiHookInstaller {
    ...
    private static func blockMatchesManagedCommand(_ block: [String], managedCommand: String?) -> Bool {
        for line in block {
            ...
            if isLegacyVibeRingHookCommand(value) {
                return true
            }
        }
        return false
    ...
    /// escape handling is unnecessary — Vibe Ring only ever writes paths,
    ...
    private static func isLegacyVibeRingHookCommand(_ command: String) -> Bool {
        let normalized = command.lowercased()
        guard normalized.contains("--source kimi") else {
            return false
        }

        return normalized.contains("viberinghooks")
