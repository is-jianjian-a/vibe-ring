...
public struct HookHealthReport: Equatable, Sendable {
    ...
    public enum Issue: Equatable, Sendable, CustomStringConvertible {
        ...
        /// Other hooks detected alongside Vibe Ring hooks (informational).
...
public enum HookHealthCheck {
    ...
    public static func checkClaude(
        ...
        // 3. Check manifest
        if fileManager.fileExists(atPath: settingsPath),
           hasVibeRingHooks(in: settingsURL, fileManager: fileManager) {
    ...
    public static func checkCodex(
        ...
        // 3. Check manifest
        if fileManager.fileExists(atPath: hooksPath),
           hasVibeRingHooks(in: hooksURL, fileManager: fileManager) {
        ...
        for (_, eventValue) in hooks {
            ...
            for group in groups {
                ...
                for hook in hookEntries {
                    ...
                    // Only check Vibe Ring / Vibe Island commands
                    let normalized = command.lowercased()
                    guard normalized.contains("viberinghooks") || normalized.contains("vibeislandhooks")
    ...
    /// Finds third-party (non-Vibe Ring) hook command names for display.
        ...
        for (_, eventValue) in hooks {
            ...
            for group in groups {
                ...
                for hook in hookEntries {
                    ...
                    // Skip Vibe Ring / Vibe Island hooks
                    if normalized.contains("viberinghooks") || normalized.contains("vibeislandhooks")
    ...
    /// Checks whether any Vibe Ring managed hooks exist in a config file.
    private static func hasVibeRingHooks(in url: URL, fileManager: FileManager) -> Bool {
