...
final class TerminalJumpServiceTests: XCTestCase {
    ...
    func testGhosttyJumpIntegrationMatchesFocusedTerminalForLiveSurfaces() throws {
        guard ProcessInfo.processInfo.environment["VIBE_RING_RUN_GHOSTTY_JUMP_INTEGRATION"] == "1" else {
            throw XCTSkip("Set VIBE_RING_RUN_GHOSTTY_JUMP_INTEGRATION=1 to run live Ghostty jump verification.")
        }
    ...
    func testCursorJumpActivatesRunningAppWithoutWorkspaceReuse() throws {
        ...
        let result = try service.jump(
            to: JumpTarget(
                terminalApp: "Cursor",
                workspaceName: "vibe-ring",
                paneTitle: "Cursor abc123",
                workingDirectory: "/Users/test/vibe-ring"
            )
        )
    ...
    func testTraeCNJumpFallsBackToWorkspaceViaTraeCLI() throws {
        ...
        XCTAssertEqual(processInvocations.values.first?.1, ["-r", "/Users/test/vibe-ring"])
    }
}
