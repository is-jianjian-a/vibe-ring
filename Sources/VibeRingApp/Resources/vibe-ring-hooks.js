// Vibe Ring plugin for OpenCode
// Bridges OpenCode events to the Vibe Ring desktop app via Unix socket.
...
const SOCKET_PATH =
  process.env.VIBE_RING_SOCKET_PATH ||
  `${process.env.HOME || homedir()}/Library/Application Support/VibeRing/bridge.sock`;
  ...
  return {
    "event": async ({ event }) => {
      try {
        ...
        // Fail open: if Vibe Ring is unavailable, don't block OpenCode
      }
    },

    "shell.env": async (input, output) => {
      output.env.VIBE_RING_ACTIVE = "1";
