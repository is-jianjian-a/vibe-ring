...
**Goal:** When a user clicks jump on a Warp-hosted Claude or Codex session in Vibe Ring, automatically focus the exact Warp tab running that session (one click, no manual tab scanning).
...
| Path | Responsibility |
|---|---|
| `Sources/VibeRingCore/WarpSQLiteReader.swift` | Read-only SQLite queries against Warp's database. Exposes `lookupPaneUUID(forCwd:)`, `currentFocusedPaneUUID()`, `tabCountInActiveWindow()`. All failures return nil/0 and log once. |
| `Sources/VibeRingApp/KeystrokeInjector.swift` | Thin wrapper over `CGEventCreateKeyboardEvent`/`CGEventPost` exposing `sendCmdShiftRightBracket()`. Protocol + default impl for injection in tests. |
| `Sources/VibeRingApp/AccessibilityPermissionChecker.swift` | One-function wrapper around `AXIsProcessTrustedWithOptions` so we can detect + graceful-degrade when Accessibility permission is missing. |
| `Tests/VibeRingCoreTests/WarpSQLiteReaderTests.swift` | Unit tests using a synthetic warp.sqlite temp-file fixture that reproduces the exact minimal schema we query. |

**Modified files**

| Path | Change |
|---|---|
| `Sources/VibeRingCore/AgentSession.swift` | Add `warpPaneUUID: String?` field to `JumpTarget` struct (and its init). Backward-compat Codable via `decodeIfPresent`. |
| `Sources/VibeRingCore/ClaudeHooks.swift` | Add `warpPaneUUID` field to `ClaudeHookPayload`. Populate it in `withRuntimeContext` when `terminalApp == "Warp"`. Pass it through `defaultJumpTarget`. |
| `Sources/VibeRingCore/CodexHooks.swift` | Same three changes as ClaudeHooks. |
| `Sources/VibeRingApp/TerminalJumpService.swift` | Add `WarpFocusedPaneReader` / `WarpTabCountReader` / `WarpKeystroker` typealiases + injection points. Add `jumpToWarpPane(_:)` private method. Register `case "dev.warp.Warp-Stable"` in the `jump(to:)` dispatcher. |
| `Tests/VibeRingCoreTests/ClaudeHooksTests.swift` | Add tests verifying `withRuntimeContext` populates `warpPaneUUID` when reader stub returns a value, skips lookup when `terminalApp != "Warp"`, and leaves payload unchanged on nil return. |
| `Tests/VibeRingCoreTests/CodexHooksTests.swift` | Same. |
| `Tests/VibeRingAppTests/TerminalJumpServiceTests.swift` | Add tests for `jumpToWarpPane` covering: already-on-target (zero keystrokes, Warp activated), reaches-target-after-N-cycles, never-reaches-target (caps out gracefully), nil target (degrades to app activation). |
...
**Files:**
- Modify: `Sources/VibeRingCore/AgentSession.swift:114-143`
- Test: `Tests/VibeRingCoreTests/SessionStateTests.swift` (add one Codable round-trip test)

- [ ] **Step 1: Write failing round-trip Codable test**

Add to `Tests/VibeRingCoreTests/SessionStateTests.swift` (inside the existing test struct):
...
Edit `Sources/VibeRingCore/AgentSession.swift` lines 114-143. The existing struct already has `tmuxTarget` and `tmuxSocketPath` — add `warpPaneUUID` alongside them:
...
```bash
git add Sources/VibeRingCore/AgentSession.swift Tests/VibeRingCoreTests/SessionStateTests.swift
...
**Files:**
- Modify: `Sources/VibeRingCore/ClaudeHooks.swift:355-403` (add field + CodingKey) and the `defaultJumpTarget` computed property near line 677
- Modify: `Sources/VibeRingCore/CodexHooks.swift` (equivalent field, CodingKey, defaultJumpTarget)
- Test: `Tests/VibeRingCoreTests/ClaudeHooksTests.swift`, `Tests/VibeRingCoreTests/CodexHooksTests.swift`

- [ ] **Step 1: Write failing test for Claude**

Add to `Tests/VibeRingCoreTests/ClaudeHooksTests.swift`:
...
Add to `Tests/VibeRingCoreTests/CodexHooksTests.swift`:
...
In `Sources/VibeRingCore/ClaudeHooks.swift`:
...
In `Sources/VibeRingCore/CodexHooks.swift`, repeat all six sub-steps from Step 4 against the Codex payload type. The `terminalApp` field is already there (added in an earlier change); `warpPaneUUID` slots in beside it.
...
```bash
git add Sources/VibeRingCore/ClaudeHooks.swift Sources/VibeRingCore/CodexHooks.swift Tests/VibeRingCoreTests/ClaudeHooksTests.swift Tests/VibeRingCoreTests/CodexHooksTests.swift
...
**Files:**
- Create: `Sources/VibeRingCore/WarpSQLiteReader.swift`
- Test: `Tests/VibeRingCoreTests/WarpSQLiteReaderTests.swift`

- [ ] **Step 1: Create the test file with a failing init test**

Create `Tests/VibeRingCoreTests/WarpSQLiteReaderTests.swift`:

```swift
import Foundation
import Testing
@testable import VibeRingCore
...
```bash
git add Sources/VibeRingCore/WarpSQLiteReader.swift Tests/VibeRingCoreTests/WarpSQLiteReaderTests.swift
...
**Files:**
- Modify: `Sources/VibeRingCore/WarpSQLiteReader.swift`
- Modify: `Tests/VibeRingCoreTests/WarpSQLiteReaderTests.swift`

- [ ] **Step 1: Add a SQLite fixture helper to the test file**

Append to `Tests/VibeRingCoreTests/WarpSQLiteReaderTests.swift` **outside** the test struct:
...
enum WarpSQLiteFixture {
    ...
    struct Scenario {
        ...
        /// tab 2 cwd=vibe-ring (pane uuid BBBB...), tab 3 cwd=/tmp (pane uuid CCCC...).
        /// Claude was launched in giftcard from Warp shell session 1001 and in vibe-ring from 1002.
        static let threeTabsTwoClaudes = Scenario(rows: [
            ...
            "INSERT INTO terminal_panes (id, uuid, cwd) VALUES " +
                ...
                "(2, x'BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB', '/Users/u/vibe-ring')," +
            ...
            "INSERT INTO commands (id, session_id, command, pwd) VALUES " +
                ...
                "(200, 1002, 'claude --dangerously-skip-permissions', '/Users/u/vibe-ring');",
...
func lookupPaneUUIDReturnsUppercaseHexForClaudeInKnownCwd() throws {
    ...
    let uuid = reader.lookupPaneUUID(forCwd: "/Users/u/vibe-ring")
...
In `Sources/VibeRingCore/WarpSQLiteReader.swift`, append a method to the struct:
...
func currentFocusedPaneUUIDReturnsActiveTabsUUID() throws {
    ...
    // Scenario has active_tab_index = 1 which is tab id 2 (vibe-ring).
    let reader = WarpSQLiteReader(databasePath: tmp)
...
**Files:**
- Modify: `Sources/VibeRingCore/ClaudeHooks.swift` (in `withRuntimeContext`, around line 920-940 where `inferTerminalApp` is called)
- Modify: `Tests/VibeRingCoreTests/ClaudeHooksTests.swift`
...
In `Sources/VibeRingCore/ClaudeHooks.swift`, find the convenience `withRuntimeContext()` (no args) that calls the full one, and find the full one that takes `environment`, `currentTTYProvider`, `terminalLocatorProvider`. Modify the full version:
...
```bash
git add Sources/VibeRingCore/ClaudeHooks.swift Tests/VibeRingCoreTests/ClaudeHooksTests.swift
...
**Files:**
- Modify: `Sources/VibeRingCore/CodexHooks.swift` (find its `withRuntimeContext` equivalent)
- Modify: `Tests/VibeRingCoreTests/CodexHooksTests.swift` (there may not be one yet — if so, see Step 1)

- [ ] **Step 1: Locate Codex's runtime-context method**

Read `Sources/VibeRingCore/CodexHooks.swift` and find the method equivalent to Claude's `withRuntimeContext`. Based on earlier exploration it's around line 389-460 and invokes `inferTerminalApp(from: environment)` (line 394). Confirm the method signature before continuing.

- [ ] **Step 2: Write failing test**

If `Tests/VibeRingCoreTests/CodexHooksTests.swift` does not exist yet, create it with the standard header:
...
In `Sources/VibeRingCore/CodexHooks.swift`, modify the full `withRuntimeContext` (around line 386) and its convenience overload (around line 378):
...
```bash
git add Sources/VibeRingCore/CodexHooks.swift Tests/VibeRingCoreTests/CodexHooksTests.swift
...
**Files:**
- Create: `Sources/VibeRingApp/KeystrokeInjector.swift`
- Test: `Tests/VibeRingAppTests/KeystrokeInjectorTests.swift`

- [ ] **Step 1: Write the failing test**

Create `Tests/VibeRingAppTests/KeystrokeInjectorTests.swift`:
...
```bash
git add Sources/VibeRingApp/KeystrokeInjector.swift Tests/VibeRingAppTests/KeystrokeInjectorTests.swift
...
**Files:**
- Create: `Sources/VibeRingApp/AccessibilityPermissionChecker.swift`
- Test: `Tests/VibeRingAppTests/AccessibilityPermissionCheckerTests.swift`

- [ ] **Step 1: Write failing test**

Create `Tests/VibeRingAppTests/AccessibilityPermissionCheckerTests.swift`:
...
```bash
git add Sources/VibeRingApp/AccessibilityPermissionChecker.swift Tests/VibeRingAppTests/AccessibilityPermissionCheckerTests.swift
...
**Files:**
- Modify: `Sources/VibeRingApp/TerminalJumpService.swift` — add new typealiases, init parameters, the `jumpToWarpPane` private method, AND the dispatcher case.
- Modify: `Tests/VibeRingAppTests/TerminalJumpServiceTests.swift`
...
// defined at file scope in Tests/VibeRingAppTests/KeystrokeInjectorTests.swift
...
Before running Step 2, confirm that `KeystrokeInjectorSpy` in `Tests/VibeRingAppTests/KeystrokeInjectorTests.swift` is at file scope (not nested inside `final class KeystrokeInjectorTests`) and has `internal` access. If Task 8 placed it nested or fileprivate, promote it before continuing this task.
...
In `Sources/VibeRingApp/TerminalJumpService.swift`, just after the existing typealiases (around line 10), add:
...
```bash
git add Sources/VibeRingApp/TerminalJumpService.swift Tests/VibeRingAppTests/TerminalJumpServiceTests.swift
...
If `/Applications/Vibe Ring.app` is running, quit it first (`osascript -e 'quit app "Vibe Ring"'`) so there's no bridge-socket collision.

- [ ] **Step 2: Grant Accessibility permission**

Open **System Settings → Privacy & Security → Accessibility**. If `VibeRingApp` is listed, toggle it on. If not, trigger a jump attempt from Vibe Ring first (which will cause macOS to add it to the list) and come back.

- [ ] **Step 3: Set up the multi-tab scenario**

1. Open Warp.
2. Open 4 new tabs, each in a **different directory**:
   - `cd ~/Developer/vibe-ring && claude`
...
1. Open Vibe Ring's session list (notch / top-bar).
...
2. In Vibe Ring, click jump for that same session.
...
1. **No permission**: temporarily disable VibeRingApp in System Settings → Accessibility. Click jump. Expected: Warp activates, no cycling, Vibe Ring toast/log says "Grant Accessibility permission to enable precision jump."
2. **Re-enable permission** and continue.
3. **Close a Claude tab** while the session is still in Vibe Ring's list. Click jump for the closed session. Expected: cycling runs the full cap, then returns "Activated Warp but could not confirm precision focus." — never crashes.
...
After committing, run a final smoke test: `swift run VibeRingApp` with three Claude tabs in Warp, click jump on each in turn, verify all three land on the right tab. If anything regressed, investigate before pushing.
