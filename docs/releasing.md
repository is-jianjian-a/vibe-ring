...
How to cut a new GitHub release for Vibe Ring.
...
2. **Build & package**:
   ...
   VIBE_RING_VERSION=<version> \
   VIBE_RING_EDDSA_PUBLIC_KEY="<your-public-key>" \
   zsh scripts/package-app.sh
   ```
   This produces `output/package/Vibe Ring.dmg` and `output/package/Vibe Ring.zip`.
...
6. **Create the release**:
   ...
   gh release create v<version> \
     "output/package/Vibe Ring.dmg#Open.Island.dmg" \
     "output/package/Vibe Ring.zip#Open.Island.zip" \
     --target main \
     --title "Vibe Ring v<version> — <Title>" \
...
2. Since this is an unsigned app, macOS will show **"Vibe Ring is damaged"** when you try to open it. Run this command in Terminal to fix it:
   由于应用未签名，macOS 会提示**「"Vibe Ring"已损坏」**。请在终端中执行以下命令：
...
| File | Purpose |
|------|---------|
| `Vibe Ring.dmg` | Styled disk image with drag-to-Applications |
| `Vibe Ring.zip` | Plain zip for automation / CI downloads |
...
```
https://raw.githubusercontent.com/Octane0411/vibe-ring/main/appcast.xml
```
...
<item>
    ...
    <enclosure
        url="https://github.com/Octane0411/vibe-ring/releases/download/vX.Y.Z/Open.Island.zip"
...
This stores the private key in your macOS Keychain and prints the public key. Save the public key — it goes into `VIBE_RING_EDDSA_PUBLIC_KEY` env var during packaging and into `SUPublicEDKey` in Info.plist.

## Signing (future)

When `VIBE_RING_SIGN_IDENTITY` is set, `package-app.sh` handles codesign + notarization automatically. At that point:
